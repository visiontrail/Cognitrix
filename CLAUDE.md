...
- `AGENT_MODE_MAX_STEPS` (default `40`), `AGENT_MODE_TIMEOUT_SECONDS` (default `600`), `AGENT_MODE_MAX_CHARTS` (default `12`) — agent-canvas-mode run budgets, independent of the Q&A limits above; exhausting any of them finalizes the run as `partial` and keeps everything already placed
...
- **Bootstrap.** On startup, when `AGENT_SKILLS_ENABLED=true`, the API verifies
  ...
  the checksum matches, it installs the skill as `anthropic/xlsx` and assigns
  it to `WriteIngestionAgent`. Idempotent on subsequent starts.
