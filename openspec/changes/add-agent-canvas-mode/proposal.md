...
Today the chat agent answers one question at a time and every chart lands on the canvas only through a manual "add to canvas" click. Lightweight analysts (the target user: someone assembling social/sales statistics into a publishable page) want to state a goal once — "build me a sales overview dashboard" — and watch the AI plan, generate, and lay out a complete web-design page end to end. The building blocks already exist (multi-chart planning + confirmation, chart asset archiving, deterministic web-design layout engine, resumable agent sessions, SSE streaming); what is missing is a long-horizon agent mode that connects them and an agent-controlled path onto the canvas.
...
- Every run **creates a fresh web-design page** ("from-zero generation"); existing canvas content is never touched. Undoing a run = deleting that page. v1 targets the web-design canvas format only; the run request carries the active canvas format so other formats can be added later, and the UI offers a one-click switch when the user is on a different format.
...
- The web-design canvas is **soft-locked** while a run is active (banner + stop button, user editing disabled).
...
- `canvas-op-streaming`: The `canvas_op` SSE event contract, client-side application of ops onto a freshly created web-design page via the deterministic layout engine, soft lock during a run, run-level undo (delete page), error placeholder blocks with retry, and reconnect replay from the op log.
