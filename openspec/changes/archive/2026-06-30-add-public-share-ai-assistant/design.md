...
The assistant file MUST contain every row available in the chart asset at publish time after `forbidden_sensitive_columns()` and `redact_rows()`. It MUST NOT silently apply `AGENT_MAX_SQL_ROWS`; that cap can still apply to chart render payloads if needed.

**Render-row fallback (amended).** The original decision was to mark the assistant unavailable for any chart that did not supply dedicated full assistant rows. In practice this permanently hid the assistant on every dashboard whose chart nodes were authored before assistant-row capture existed — those nodes carry render rows but no `assistant_rows`, and republishing alone could never enable the assistant. The implementation now falls back to the chart's full render rows as the assistant source when no explicit assistant rows are provided. In the normal agent flow the render rows and assistant rows derive from the same `rows`/`data` payload, so fidelity loss is minimal; the fallback rows pass through the identical `forbidden_sensitive_columns()` / `redact_rows()` pipeline. A chart is only assistant-unavailable when it has no rows at all, and the publish endpoint already rejects charts with empty rows (`PUBLISH_CHART_DATA_REQUIRED`) — so the page-level assistant is effectively available whenever the snapshot has at least one chart node.
